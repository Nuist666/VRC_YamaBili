...
| Estimated IDs per day | `30000` | 预计每天增加的编号数量，须大于 0；可用两次观测编号之差除以间隔天数估算 |
| Days to cover (+20% reserve) | `30` | 计划覆盖天数，须为正整数；计算时自动增加 20% 增长余量 |
| Apply coverage estimate | — | 根据上述输入和 First record ID 更新 URL count；此按钮不会烘焙或上传 |
| First record ID | `550000` | 池内最小编号，须为正整数且不大于最新观测编号；保留较早的起点可继续覆盖旧记录 |
| URL count (max 2,000,000) | `1200000` | 新模块默认数量，覆盖 `550000`–`1749999`；允许 `1`–`2000000` |
...
资源开销参考：10 万条六位编号地址的 UTF-16 字符内容约 10.2 MB，120 万条约 122 MB（七位编号会略增），真实开销更大；**扩大范围会增加资源占用**。
