...
| URL count (max 2,000,000) | `210003` | 新模块默认数量，覆盖 `600000`–`810002`；允许 `1`–`2000000`；**超过 `220000` 会显示资源占用可能过高的警告** |
...
**URL count 超过 `220000` 时会显示黄色警告**（资源占用可能过高），并按 105 字节/条给出「当前约 xx MB，而推荐上限约 23 MB」的对比估算。这是提示而非硬性限制：上限仍是 200 万条，超过推荐值时请确认世界体积、构建时间与客户端内存可接受再烘焙。
